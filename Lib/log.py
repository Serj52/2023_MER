...
def __init_logs(config):
    ...
    last_log_path = os.path.join(config.logs_folder_path, f'last_{config.log_file}')

    # Если нет директории для логов - создаю
    if not os.path.isdir(config.logs_folder_path):
        os.makedirs(config.logs_folder_path)
...
def set_1(config):
    ...
    logging.basicConfig(
        ...
        datefmt="%Y-%m-%d %H:%M:%S",
        handlers=[
            logging.handlers.TimedRotatingFileHandler(os.path.join(config.folder_logs, config.log_file), when='midnight'),
...
def set_2(config):
    ...
    logging.basicConfig(
        ...
        datefmt="%d.%m.%Y %H:%M:%S",
        handlers=[
            logging.handlers.RotatingFileHandler(os.path.join(config.logs_folder_path, config.log_file),
                                                 maxBytes=int(1e+7), backupCount=1),
            logging.handlers.RotatingFileHandler(os.path.join(config.logs_folder_path, f'last_{config.log_file}')),
            logging.StreamHandler(stream=sys.stdout)]
    )
